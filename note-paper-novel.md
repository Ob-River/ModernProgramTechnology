...
>是否是“设置”弹出窗口没一一写到，有的省略了。文字，图片，链接，代码，。窗（能调整window长宽的）：活动窗口，卡窗，file choose window；非窗：选项页，选项卡，选项口，设置。布尔选项，多选项，下选框，文本框。
>加载项，宏，miBA，COM，。
# 使用流程
## 1.note  
## 2.doc
...
### 账户，反馈，教程。
### 设置：
#### 常用 显示 校对 保存 版式 高级 界面定义 加载项 安全与权限 资源（格式，样式，字体，媒体，色卡，引用，表，域，宏，嵌入程序）。
## 开始
>star dot
### 1.剪贴版
...
### <div id="1">3.段落</div>
编号，行间距，左对齐Ctrl+L，居中Ctrl+E，右对齐ctrl+R，行、段的间距，底纹，边框，多级列表，排序，增加缩进，减少缩进，显示或隐藏编辑标记，多级列表，排序，中文版式选项卡（横纵混排，双行合一，合并字符，调整宽度，字符缩放，横纵混排）段落设置。
### 4.样式
样式选项口，更改样式选项卡，样式卡窗。
### 5.编辑
...
封面，封面选项卡，空白页，分页。
### 2. 表格
划表格，插入表格设置，文转表格，（Excel全局窗口），绘制表格，快速表格，沉浸表格。
...
#### 大纲
1，大纲工具
<<（），<（），正文文本下选框，>（），>>（），▲（），▼（），+（），-（）。显示级别(S)下选框，显示文本格式，仅显示首行。
2，主控/子 文档
